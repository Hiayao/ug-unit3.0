<template>
  <div class="All">
    ...
    <div style="margin-bottom: 10px">
      ...
      <span @click="reset" class="box"><a-button>重 置</a-button></span>
      <span @click="delayAdd" class="box"><a-button>延迟加</a-button></span>
      {{ current }}
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    const { current, addCount, cutCount, setCount,resetCount,delayAddCount } = useCount(data.num);
    const add = (): void => {
    ...
    const set = (): void => {
      // setCount()传一个参数，点击后立马变为该值，默认为50
      setCount(100);
    };
    const reset = (): void => {
      resetCount()
